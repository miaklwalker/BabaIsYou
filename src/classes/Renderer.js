...
export default class Renderer{
    ...
    tint=(canvas ,context ,sprite, img, index)=>{
        ...
        for(let i = 0; i < spriteData.data.length ; i+=4){
            if(    spriteData.data [i]   > start
                && spriteData.data [i+1] > start
                && spriteData.data [i+2] > start
            ){
                spriteData.data [i]   = r
                spriteData.data [i+1] = g
                spriteData.data [i+2] = b
                spriteData.data [i+3] = a
            }else if(spriteData.data [i] > cutoff
                && spriteData.data [i+1] > cutoff
                && spriteData.data [i+2] > cutoff
            ){
